...
This object is part of the `Reaction` object and describes a species (SmallMolecule, Protein, Complex) participating in the reaction. THE TYPE OF THE REACTION ELEMENT IS SPECIFIED IN THE TYPE FIELD. The stochiometry is of the species is specified in the `stoichiometry` field, whereas negative values indicate that the species is a reactant and positive values indicate that the species is a product of the reaction.

- **species_id**: Identifier
  - Description: Internal identifier to either a protein or reactant defined in the EnzymeML Document.
  - Term: schema:identifier
- **stoichiometry**: float
  - Description: Float number representing the associated stoichiometry. Negative values indicate that the species is a reactant and positive values indicate that the species is a product of the reaction.
...
The **Equation** object describes a mathematical equation used to model a reaction. The equation can be one of several types:
...
| **Assignment**   | Expresses a variable as an algebraic function of other variables. | `educt = educt_0 - product` |
| **Initial Assignment**   | HERE A DESCRIPTION IS MISSING | `HERE AN EXAMPLE IS MISSING`
